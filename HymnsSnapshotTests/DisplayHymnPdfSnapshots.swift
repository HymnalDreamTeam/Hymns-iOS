...
class DisplayHymnPdfSnapshots: XCTestCase {
    ...
    func test_loading() {
        let view = ScrollView(showsIndicators: false) {
            DisplayHymnPdfView(viewModel: DisplayHymnPdfViewModel(url: URL(string: "http://www.dummylink.com")!))
        }.ignoresSafeArea()
    ...
    func test_displayError() {
        ...
        let view = ScrollView(showsIndicators: false) {
            DisplayHymnPdfView(viewModel: errorViewModel)
        }.ignoresSafeArea()
        assertVersionedSnapshot(matching: view, as: .image(layout: .sizeThatFits))
    }
    ...
    func test_displayPdf() {
        let view = ScrollView(showsIndicators: false) {
            DisplayHymnPdfView(viewModel: DisplayHymnPdfViewModel(preloader: preloader, url: URL(string: "/en/hymn/h/1151/f=gtpdf")!))
        }.ignoresSafeArea()
        assertVersionedSnapshot(matching: view, as: .image(layout: .sizeThatFits))
    }
}
